use crate::{
    ...
    resources::{Outbox, PendingMessages, ServerState, SimulationTime, insert_resources},
    systems::{
        physics_movement, process_connected, process_disconnected, process_messages,
        replicate_snapshot_batch,
    },
};
...
impl ServerGame for SandboxGame {
    ...
    fn connected(&mut self, id: ClientId) {
        process_connected(id, &mut self.world);
    }
    fn disconnected(&mut self, id: ClientId) {
        process_disconnected(id, &mut self.world);
    }
    fn handle_message(&mut self, id: ClientId, message: ClientMessage) {
        self.world
    ...
    fn drain_outgoing(&mut self) -> Vec<Outgoing<ServerMessage>> {
        std::mem::take(&mut self.world.resource_mut::<Outbox>().0)
    }
